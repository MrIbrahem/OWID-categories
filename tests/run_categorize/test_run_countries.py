...
class TestProcessFiles:
    ...
    def test_resolve_category_redirect_is_called_with_built_category(self):
        """Test that resolve_category_redirect is invoked with the category built from entity name."""
        mock_site = Mock()
        mock_page = MagicMock()
        mock_page.exists = True
        mock_page.text.return_value = "Some page text"
        ...
        test_data = {
            ...
            "country": "Canada",
            "graphs": [
                {"title": "File:Test.svg", "indicator": "GDP"},
            ],
        }

        captured_args = []

        def capture_redirect(site, category):
            captured_args.append((site, category))
            return category  # pass-through

        with patch("run_categorize.load_json_file", return_value=test_data):
            with patch("run_categorize.get_category_member_count", return_value=0):
                with patch("run_categorize.resolve_category_redirect", side_effect=capture_redirect):
                    process_files(mock_site, COUNTRIES_DIR / "CAN.json", dry_run=True)

        assert len(captured_args) == 1, "resolve_category_redirect should be called once per process_files call"
        _, called_category = captured_args[0]
        assert called_category == "Category:Our World in Data graphs of Canada"

    def test_process_files_uses_redirected_category(self):
        """Test that the category returned by resolve_category_redirect is used for page edits."""
        mock_site = Mock()

        redirected_category = "Category:Our World in Data graphs of Canada (redirected)"
        ...
        def capture_add_category(site, title, category, dry_run):
            added_categories.append(category)
            return True

        with patch("run_categorize.load_json_file", return_value=test_data):
            with patch("run_categorize.get_category_member_count", return_value=0):
                with patch("run_categorize.resolve_category_redirect", return_value=redirected_category):
                    with patch("run_categorize.add_category_to_page", side_effect=capture_add_category):
                        with patch("run_categorize.ensure_category_exists", return_value=True):
                            with patch("run_categorize.get_category_members", return_value=[]):
                                process_files(mock_site, COUNTRIES_DIR / "CAN.json", dry_run=True)

        assert any(cat == redirected_category for cat in added_categories), (
            "The redirected category name should be used when adding categories to pages"
        )
